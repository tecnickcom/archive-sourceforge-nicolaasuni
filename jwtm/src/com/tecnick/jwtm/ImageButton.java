...
extends Panel {
	 ...
	 * serialVersionUID
	 */
	private static final long serialVersionUID = 6436676886270843900L;
	...
	public ImageButton(Image image, String label) {
		...
		imageButtonMouseAdapter imageButtonMouseAdapter = new imageButtonMouseAdapter();
		this.addMouseListener(imageButtonMouseAdapter);
		actionCommand = "void";
	 ...
	 */
	public void setStateColors(Color ba, Color bb, Color bc, Color ta, Color tb,
			Color tc, Color sa, Color sb, Color sc) {
		col_off_bck = ba;
		col_over_bck = bb;
		col_on_bck = bc;
	...
	public void resize() {
		...
		int w, h;
		w = iw + padding[0] + padding[1] + (2 * bevel_width);
		h = ih + padding[2] + padding[3] + (2 * bevel_width);
	 ...
	 * @param gbuffer current graphic context
	 */
...
//		by doing this everything that is drawn by bufferGraphics will be written on the offscreen image.
		...
		int xblockpos = (int) ( (w + borderL - borderR - iw - i_gap -
				lw) / 2);
		if (pos == RIGHT) {
			if (center_block) {
				ix = xblockpos;
			...
			if (pos == LEFT) {
				if (center_block) {
					...
					lx = w - borderL - iw - i_gap - lw;
				}
				ix = lx + lw + i_gap;
			}
			else { // TOP or BOTTOM
				//center image horizzontally
				ix = (int) ( (w + borderL - borderR - iw) / 2);
				lx = (int) ( (w + borderL - borderR - lw) / 2);
			}
		}
...
//	-----------------------------------------------------------------------------
	...
	protected void imageButton_MousePushed() {
		isButtonPushed = true;
		setButtonComponents(audio_click, col_on_txt, col_on_bck, col_on_sdw, img_bg_on, img_on);
	}
	...
	protected void imageButton_MouseOver(boolean sound_on) {
		...
		setButtonComponents(sound, col_over_txt, col_over_bck, col_over_sdw, img_bg_over, img_over);
	}
	...
	protected void imageButton_MouseOff() {
		isButtonPushed = false;
		AudioClip sound = null;
		setButtonComponents(sound, col_off_txt, col_off_bck, col_off_sdw, img_bg_off, img_off);
	}
