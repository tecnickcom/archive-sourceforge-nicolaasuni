<?php
//============================================================+
// File name   : cp_edit_icons_client.php                      
// Begin       : 2002-03-21                                    
// Last Update : 2008-07-06
//                                                             
// Description : Edit Menu Client Icons                        
//                                                             
//
// Author: Nicola Asuni
//
...
$pagelevel = K_AUTH_ADMIN_CP_EDIT_ICONS_CLIENT;
require_once('../../shared/code/cp_authorization.'.CP_EXT);

$thispage_title = $l['t_client_icon_editor'];


// Initialize variables
if(isset($icon_link) AND (!empty($icon_link))) {
	$size = GetImageSize(K_PATH_IMAGES_ICONS_CLIENT.$icon_link);
	$icon_width = $size[0];
	$icon_height = $size[1];
} else {
	$icon_link = "";
...
switch($menu_mode) {
	...
	case $l['w_delete']:{
		...
		$sql = "DELETE FROM ".K_TABLE_ICONS_CLIENT." WHERE icon_id=".$icon_id."";
	...
	case $l['w_update']:{ // Update icon
		if($formstatus = F_check_form_fields()) {
			...
			if(!F_check_unique(K_TABLE_ICONS_CLIENT, "icon_name='".$icon_name."'", "icon_id", $icon_id)) {
				F_print_error("WARNING", $l['m_duplicate_name']);
				$formstatus = FALSE; F_stripslashes_formfields();
			}
			else {
				if($_FILES['userfile']['name']) {
					$icon_link = F_upload_file("userfile", K_PATH_IMAGES_ICONS_CLIENT);
					$size = GetImageSize(K_PATH_IMAGES_ICONS_CLIENT.$icon_link);
					$icon_width = $size[0];
					$icon_height = $size[1];
				}
				$sql = "UPDATE IGNORE ".K_TABLE_ICONS_CLIENT." SET 
				icon_name='".$icon_name."', 
				icon_link='".$icon_link."', 
	...
	case $l['w_add']:{ // Add icon
		if($formstatus = F_check_form_fields()) {
			...
			$sql = "SELECT icon_name FROM ".K_TABLE_ICONS_CLIENT." WHERE icon_name='".$icon_name."'";
			if(($r = F_aiocpdb_query($sql, $db))AND(F_aiocpdb_num_rows($r))) {
				...
				$sql = "INSERT IGNORE INTO ".K_TABLE_ICONS_CLIENT." (
				icon_name, 
				icon_link, 
				icon_width, 
				icon_height
...
if($formstatus) {
	if (($menu_mode != $l['w_clear']) AND ($menu_mode != unhtmlentities($l['w_clear']))) {
		if(!isset($icon_id) OR (!$icon_id)) {
			$sql = "SELECT * FROM ".K_TABLE_ICONS_CLIENT." ORDER BY icon_name LIMIT 1";
		}	else {
			$sql = "SELECT * FROM ".K_TABLE_ICONS_CLIENT." WHERE icon_id=".$icon_id." LIMIT 1";
		}
		if($r = F_aiocpdb_query($sql, $db)) {
...
<b><?php echo F_display_field_name_link('w_icon', 'h_icon_select', "selectWindow=window.open('cp_select_icons_client.".CP_EXT."?formname=form_iconeditor&amp;idfield=icon_id&amp;fieldtype=0&amp;fsubmit=1','selectWindow','dependent,height=450,width=450,menubar=no,resizable=yes,scrollbars=yes,status=no,toolbar=no')"); ?></b>
</td>
<td class="fillOE">
...
<td class="fillOE" rowspan="2" align="right" valign="top"><img name="imageicon" src="<?php echo K_PATH_IMAGES_ICONS_CLIENT; ?><?php echo $icon_link; ?>" border="0" alt="" width="<?php echo $icon_width; ?>" height="<?php echo $icon_height; ?>" /></td></tr>
<!-- END SELECT ICON ==================== -->
...
$handle = opendir(K_PATH_IMAGES_ICONS_CLIENT);
...
function FJ_show_icon (){
	document.images.imageicon.src= "<?php echo K_PATH_IMAGES_ICONS_CLIENT; ?>"+document.form_iconeditor.icon_link_dir.options[document.form_iconeditor.icon_link_dir.selectedIndex].value;
}

function FJ_show_icon2 (){
	document.images.imageicon.src= "<?php echo K_PATH_IMAGES_ICONS_CLIENT; ?>"+document.form_iconeditor.icon_link.value;
}
