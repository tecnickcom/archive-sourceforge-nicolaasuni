<?php
//============================================================+
// File name   : cp_edit_language_templates.php                
// Begin       : 2001-09-30                                    
// Last Update : 2008-07-06
//                                                             
// Description : Edit words templates in all enabled languages 
//                                                             
//
// Author: Nicola Asuni
//
...
$pagelevel = K_AUTH_ADMIN_CP_EDIT_LANGUAGE_TEMPLATES;
require_once('../../shared/code/cp_authorization.'.CP_EXT);

$thispage_title = $l['t_language_editor'];
...
switch($menu_mode) {
	...
	case "forcedelete":{ 
		if($forcedelete == $l['w_delete']) { //check if delete button has been pushed (redundant check)
			$sql = "DELETE FROM ".K_TABLE_LANGUAGE_DATA." WHERE word_id='".$word_id."'";
	...
	case $l['w_update']:{ // Update word
		if($formstatus = F_check_form_fields()) {
			...
			if(!F_check_unique(K_TABLE_LANGUAGE_DATA, "word_id='".$newword_id."'", "word_id", $word_id)) {
				...
				$sql = "UPDATE IGNORE ".K_TABLE_LANGUAGE_DATA." SET ".$SETstring." WHERE word_id='".$word_id."'";
				if(!$r = F_aiocpdb_query($sql, $db)) {
					F_display_db_error();
				}
				if ($word_id != $newword_id) { //change template ID
					$sql = "UPDATE IGNORE ".K_TABLE_LANGUAGE_DATA." SET word_id='".$newword_id."' WHERE word_id='".$word_id."'";
					if(!$r = F_aiocpdb_query($sql, $db)) {
						F_display_db_error();
					}
					$word_id = $newword_id;
	...
	case $l['w_add']:{ // Add word
		if($formstatus = F_check_form_fields()) {
			//check if word_id is unique
			$sql = "SELECT word_id FROM ".K_TABLE_LANGUAGE_DATA." WHERE word_id='".$newword_id."'";
			if(($r = F_aiocpdb_query($sql, $db))AND(F_aiocpdb_num_rows($r))) {
				...
				$sql = "INSERT IGNORE INTO ".K_TABLE_LANGUAGE_DATA." (word_id ".$FIELDSstring.") VALUES ('".$newword_id."' ".$VALUESstring.")";
				if(!$r = F_aiocpdb_query($sql, $db)) {
					F_display_db_error();
				}
			}
...
if($formstatus) {
	if (($menu_mode != $l['w_clear']) AND ($menu_mode != unhtmlentities($l['w_clear']))) {
		if(!isset($word_id) OR (!$word_id)) {
			$sql = "SELECT * FROM ".K_TABLE_LANGUAGE_DATA." ORDER BY word_id LIMIT 1";
		} else {
			$sql = "SELECT * FROM ".K_TABLE_LANGUAGE_DATA." WHERE word_id='".$word_id."' LIMIT 1";
		}
		if($r = F_aiocpdb_query($sql, $db)) {
...
<!-- ====================================================== -->
<form action="<?php echo $_SERVER['SCRIPT_NAME']; ?>" method="post" enctype="multipart/form-data" name="form_languageeditor" id="form_languageeditor">

<!-- comma separated list of required fields -->
<input type="hidden" name="ff_required" id="ff_required" value="newword_id" />
...
<td class="fillOE">
<select name="word_id" id="word_id" size="0" onchange="document.form_languageeditor.submit()">
<?php
$sql = "SELECT word_id FROM ".K_TABLE_LANGUAGE_DATA." ORDER BY word_id";
if($r = F_aiocpdb_query($sql, $db)) {
	while($m = F_aiocpdb_fetch_array($r)) {
		echo "<option value=\"".$m['word_id']."\"";
		if($m['word_id'] == $word_id) {
			echo " selected=\"selected\"";
		}
		echo ">".$m['word_id']."</option>\n";
	}
}
else {
	F_display_db_error();
...
<td class="fillOE"><input type="text" name="newword_id" id="newword_id" value="<?php echo $newword_id; ?>" size="50" /></td>
</tr>
...
<?php //show buttons
if ($word_id) {
	F_submit_button("form_languageeditor","menu_mode",$l['w_update']); 
	F_submit_button("form_languageeditor","menu_mode",$l['w_delete']); 
}
F_submit_button("form_languageeditor","menu_mode",$l['w_add']); 
F_submit_button("form_languageeditor","menu_mode",$l['w_clear']); 
?>
</td>
</tr>
</table>
...
function FJ_get_field_index(what) {
	for (var i=0;i<document.form_languageeditor.elements.length;i++) {
		if(what == document.form_languageeditor.elements[i]) {
			return(i+1);
		}
	}
	return (-1);
