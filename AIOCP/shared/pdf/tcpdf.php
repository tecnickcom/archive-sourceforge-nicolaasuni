...
require_once(dirname(__FILE__).'/tcpdf_fonts.php');

// TCPDF static color methods and data
require(dirname(__FILE__).'/tcpdf_colors.php');

// TCPDF static image methods and data
require(dirname(__FILE__).'/tcpdf_images.php');

// TCPDF static methods and data
require_once(dirname(__FILE__).'/tcpdf_static.php');
...
class TCPDF {
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		if (preg_match(TCPDF_FONTS::$uni_RE_PATTERN_ARABIC, $s)) {
			$arabic = true;
		} else {
			$arabic = false;
		}
		// check if string contains RTL text
		if ($arabic OR ($this->tmprtl == 'R') OR preg_match(TCPDF_FONTS::$uni_RE_PATTERN_RTL, $s)) {
			$rtlmode = true;
		} else {
			$rtlmode = false;
		}
	...
	protected function _puttruetypeunicode($font) {
		...
		$stream = TCPDF_FONTS::$uni_identity_h;
		// ToUnicode Object
		$this->_newobj();
