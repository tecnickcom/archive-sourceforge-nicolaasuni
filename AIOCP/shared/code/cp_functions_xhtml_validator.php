<?php
//============================================================+
// File name   : cp_functions_xhtml_validator.php              
// Begin       : 2002-05-25                                    
// Last Update : 2005-03-22                                   
//                                                             
// Description : Validate XHTML code and convert HTML tags to  
//               XHTML                                         
//				 preserve other languages tags                 
//               (php, asp, jsp, ...)                          
//               Do not check headers, only the general rules  
//               for tags, attributes and nesting              
//
// Author: Nicola Asuni
//
// (c) Copyright:
...
//============================================================+
	 ...
	 * @param doc_charset is the document charset encoding and is passed as a reference 
	 ...
	 * @param encoding String document encoding (e.g.: "iso-8859-1")
	 * @return String the transcoded text
	 */
	function F_validate_xhtml($code_to_clean, &$doc_charset, $indent = true, $entities_off = false, $encoding = "iso-8859-1") {
		global $l, $db;
		...
		$code_to_clean = unhtmlentitiest($code_to_clean, true);
		
		if ($encoding == "UTF-8") {
			// replace extended UTF-8 characters with html numeric entities
			$code_to_clean = htmlentitiesUTF8($code_to_clean);
		}
			...
			if ( ($currentchar == "<") AND (eregi("[[:alpha:]]", substr($code_to_clean, $i+1, 1))) ) { //we are inside a tag
				...
				if ($j>0) {
					if (!strcasecmp(substr($tag_list[$j], 1),$tag_list[$j-1])) {
						...
						$sql = "SELECT tag_endtag FROM ".K_TABLE_XHTML_TAGS." WHERE tag_name='".$lct_name."'";
				...
				$sql = "SELECT tag_endtag FROM ".K_TABLE_XHTML_TAGS." WHERE tag_name='".$tag_list[$j]."'";
				$emptyelement = false;
				if($r = F_aiocpdb_query($sql, $db)) {
					if($m = F_aiocpdb_fetch_array($r)) {
						$emptyelement = !$m['tag_endtag'];
					}
				}
				else {
					F_display_db_error();
					...
					if (!$subtag) { //we are not inside a subtag
						if (eregi("[[:alpha:]]", $currentchar)) { //attribute found
							...
							if (!$attribdef) {
								...
								$sql = "SELECT htmattrib_name FROM ".K_TABLE_XHTML_ATTRIBUTES." WHERE htmattrib_name='".$attributename."=\"".$attributename."\"'";
		...
		if ($entities_off) {
			// remove htmlentities
			$xhtml_code = unhtmlentitiest($xhtml_code, true);
		}
	 ...
	 */
	function unhtmlentitiest($text_to_convert, $preserve_tagsign = true) {
		...
		$return_text = preg_replace_callback('/\&\#([0-9]+)\;/m', create_function('$matches', 'return unicodeToChars($matches[1]);'), $return_text);
		$return_text = preg_replace_callback('/\&\#x([0-9A-Fa-f]+)\;/m', create_function('$matches', 'return unicodeToChars(intval($matches[1], 16));'), $return_text);
		return $return_text;
	}
